...
struct SettingsView: View {
        ...
        NavigationView {
            List {
                ...
                Section {
                    ...
                    NavigationLink(destination: AnimationSettingsView(settings: settings)) {
                        SettingRow(
                            icon: "clock",
                            title: "Reading Speed",
                            subtitle: speedDescription
                        )
                    }
                } header: {
    ...
    private var speedDescription: String {
        if settings.typingSpeed < 0.05 {
            return "Fast"
        } else if settings.typingSpeed < 0.08 {
            return "Normal"
        } else {
            return "Slow"
        }
...
struct ThemeSettingsView: View {
    ...
    private func themeButton(_ title: String, color: Color, tag: Int) -> some View {
        Button(action: { settings.selectedTheme = tag }) {
            HStack {
                Text(title)
                Spacer()
                Circle()
    ...
    var body: some View {
        List {
            Section {
                ...
                Text("Adjust how fast the story text appears on screen")
            }
        }
