...
struct SpacePioneerStoryView: View {
    @ObservedObject var settings: SettingsModel
    @StateObject private var gameState = SpacePioneerGameState()
    @Environment(\.dismiss) var dismiss
    @State private var selectedChoice: Int? = nil
    @State private var currentText: String = ""
    @State private var showingConsequence = false
    @State private var previousScenarios: [SpacePioneerScenario] = []
            ...
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 24) {
                        VStack(alignment: .leading, spacing: 24) {
                            if gameState.currentScenario == .firstContact && storyHistory.isEmpty {
                                Text("Your space odyssey begins.")
                                    .font(.headline)
                                    .padding(.bottom, 8)
                            }
    ...
    private func navigateToNext(choice: SpacePioneerChoice) {
        withAnimation {
